...
[The remainder is verbatim as in your screenshot.]
`,
  reference: [
    {
      title: "Boson CCNA Curriculum 200-301, Module 9: Automation and Programming",
...
[...rest is verbatim as in your screenshot...]
`,
  reference: [
    {
      title: "Boson CCNA Curriculum 200-301, Module 8: Security Fundamentals",
...

{
  ...
  correctAnswer: 5,
  explanation: `
If the Open Shortest Path First (OSPF) router ID of a router is not manually configured, the router ID is determined first by the highest Internet Protocol (IP) address among loopback interfaces, then by the highest IP address among physical interfaces if no loopback interfaces have been configured. To manually configure a router ID for an OSPF router, you should issue the router-id value command in router configuration mode, where value is a 32-bit value in dotted decimal notation. If the router ID is not manually configured, the router ID will be the highest loopback IP address configured on a router. If a loopback IP address is not configured, the router ID will be the highest IP address among configured interfaces on the router. 

Because loopback interfaces are logical, not physical, a loopback interface can never fail. For this reason, configuring a loopback interface is recommended for OSPF operation, because loopback interfaces are more reliable than physical interfaces.
...
The OSPF priority value is not used for determining the router ID. The priority value is used for electing the designated router (DR) and backup designated router (BDR) for an OSPF multiaccess network. [...additional explanation as in your screenshot...]
`,
  reference: [
    {
      title: "Boson CCNA Curriculum 200-301, Module 6: IP Routing",
